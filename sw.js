...
self.addEventListener('fetch',event=>{
  ...
  if(u.origin===self.location.origin&&(u.pathname.startsWith('/wellone-db/')||u.pathname.startsWith('/supabase/')||u.pathname==='/api/supabase-proxy'||u.pathname.startsWith('/.netlify/functions/supabase-proxy')))return;
  ...
  const cdn=u.hostname==='cdn.jsdelivr.net'&&r.destination==='script';
  const image=r.destination==='image'||publicStorage(u);
  if(code||cdn){event.respondWith(networkFirst(r));return;}
